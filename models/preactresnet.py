...
class PreActResnetBasicBlock(nn.Module):
  ...
  def forward(self, x):
    ...
    if self.in_channels != self.out_channels: # 채널 수 맞추기(zero padding)
      zero_block = torch.zeros(shortcut.shape[0], self.out_channels - self.in_channels, shortcut.shape[2], shortcut.shape[3], device = shortcut.device)
...
# Predefined preactresnet encoders
def preactresnet20_encoder():
    return PreActResNetEncoder(n=3, block=PreActResnetBasicBlock)

def preactresnet32_encoder():
    return PreActResNetEncoder(n=5, block=PreActResnetBasicBlock)
