...
class MoCo(nn.Module):
    def __init__(self, encoder, version='v2', proj_hidden_dim=512, proj_output_dim=128, 
        ...
        # queue: stores embedded negative keys, shape: (proj_output_dim, queue_size)
        self.register_buffer("queue", F.normalize(torch.randn(proj_output_dim, self.queue_size), dim=0))
            
        # queue pointer: indicates the current position in the queue (FIFO behavior)
        self.register_buffer("queue_ptr", torch.zeros(1, dtype=torch.long))
    
    @torch.no_grad()
    def momentum_update(self):
        # "θ_k" is replaced with "momentum · θ_k + (1 − momentum) · θ_q"
        for q_param, k_param in zip(self.q_encoder.parameters(), self.k_encoder.parameters()):
            k_param.copy_(self.momentum * k_param + (1 - self.momentum) * q_param)
        
        for q_param, k_param in zip(self.q_head.parameters(), self.k_head.parameters()):
            k_param.copy_(self.momentum * k_param + (1 - self.momentum) * q_param)
